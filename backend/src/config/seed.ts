...
export async function seedDatabase() {
  ...
  try {
    ...
    // -------------------------------------------------------------------------
    // 2. Criar Estrutura de Canais (Nós) — apenas se ainda não existirem
    // -------------------------------------------------------------------------
...
Consultar anexos correspondentes no Manual de Estágio.`;
      ...
      await ChatNode.create({ title: '3.5. Estágio (MARH)', content: 'Carga: 180 horas. Pode iniciar a partir do 4º semestre.', parentId: marh.id });
      ...
      console.log('✅ Árvore de canais populada com conteúdo integral.');
    }

    // -------------------------------------------------------------------------
    // 3. Criar Logs de Exemplo para o Dashboard (apenas se ainda não existirem)
    // -------------------------------------------------------------------------
